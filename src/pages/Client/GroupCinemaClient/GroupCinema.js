        ...
        return lstGroupCinemas?.map((cumRap, index) => {
            return <div key={index} className="xl:w-1/4 md:w-1/3 sm:w-1/2 p-4 " >
                <div className="bg-gray-100 py-2 px-3 rounded-lg cursor-pointer" onClick={() => {
                    ...
                    <div style={{ backgroundImage: `url(${DOMAIN_STATIC_FILE}${cumRap.logo})`, backgroundPosition: 'center', backgroundSize: 'cover',height:330 }}>
                        <img style={{
                            objectFit:'cover',
                        }} className="h-full opacity-0 rounded w-full object-cover object-center mb-2" src={`${DOMAIN_STATIC_FILE}${cumRap.logo}`} alt={cumRap.logo} />
                    </div>
