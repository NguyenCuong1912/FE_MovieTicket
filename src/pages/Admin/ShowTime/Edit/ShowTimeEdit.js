...
import {
  capNhatLichChieuAction,
  chiTietLichChieuAction,
...
export default function ShowTimeEdit(props) {
  ...
  useEffect(async () => {
    ...
    try {
      const result = await quanLySeatsServices.getPriceSeat(
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      idRoom: Yup.string().required("Không được trống !"),
      idCinema: Yup.string().required("Không được trống !"),
      idFilm: Yup.string().required("Không được trống !"),
      showDate: Yup.string().required("Không được để trống !"),
    }),
    onSubmit: (values) => {
      ...
      >
        ...
        <Form.Item label="Cụm Rạp">
          ...
          >
            {lstGroupCinemas.map((groupCinema, index) => {
              return (
                <Option key={index} value={groupCinema.id}>
                  {groupCinema.groupName}
                </Option>
              );
            })}
          </Select>
        ...
        <Form.Item label="Ngày chiếu">
          <DatePicker
            ...
            value={moment(formik.values.showDate)}
            showTime
            onOk={onOk}
          />
