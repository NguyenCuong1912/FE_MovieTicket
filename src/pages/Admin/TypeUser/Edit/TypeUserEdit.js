...
import {
  capNhatLoaiNguoiDungAction,
  chiTietLoaiNguoiDungAction,
} from "../../../../redux/Actions/QuanLyLoaiNguoiDungAction";
import { Form, Input } from "antd";
import { useFormik } from "formik";
import * as Yup from "yup";
export default function TypeUserEdit(props) {
  const dispatch = useDispatch();
  const { userTypeEdit } = useSelector(
    (state) => state.QuanLyLoaiNguoiDungReducer
  );
  useEffect(() => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      type: Yup.string().required("Không được trống !"),
      nameType: Yup.string().required("Không được trống !"),
    }),
    onSubmit: (values) => {
