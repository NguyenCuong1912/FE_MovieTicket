...
export default function FilmCreate(props) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      nameFilm: Yup.string().required("Không được trống !"),
      trailer: Yup.string().required("Không được trống !"),
      description: Yup.string().required("Không được trống !"),
      actor: Yup.string().required("Không được trống !"),
    }),
    onSubmit: (values) => {
    ...
    <div>
      ...
      >
        ...
        <Form.Item label="Đạo diễn">
          <Input name="actor" onChange={formik.handleChange} />
          {formik.errors.actor && formik.touched.actor && (
            <p className="m-0 mt-1 text-red-600">{formik.errors.actor}</p>
          )}
        </Form.Item>
        ...
        <Form.Item label="Đánh Giá">
          <InputNumber
            ...
            min="1"
            max="10"
            onChange={handleNumberAndSwitch("rate")}
          />
        </Form.Item>
        ...
        <Form.Item label="Sắp chiếu" valuePropName="checked">
          <Switch
            name="comingSoon"
            onChange={handleNumberAndSwitch("comingSoon")}
          />
        </Form.Item>
        <Form.Item label="Đang Chiếu" valuePropName="checked">
          <Switch
            name="nowShowing"
            onChange={handleNumberAndSwitch("nowShowing")}
          />
        </Form.Item>
        <Form.Item label="Ảnh">
          <input
