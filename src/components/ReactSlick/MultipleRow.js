...
import { ALL_PHIM, PHIM_DANG_CHIEU, PHIM_SAP_CHHIEU, SET_PHIM } from '../../redux/Types/QuanLyPhimType';
...
const typeNormapBtn ="bg-white p-2 text-gray-700 border border-gray-700 rounded mr-3"
            ...
            {optionFilms?.map((item, index) => {
                return (
                    <div key={item?.idx} className="ml-3 mb-3">
                        ...
                        className={isActive === item?.idx ? activeBtn : typeNormapBtn}>{item?.title}</button>
                    </div>
                )
            })}
            </div>
